    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for position in self.legalPositions:
                ...
                allPossible[position] = emissionModel[distance] * self.beliefs.get(position)
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for position, value in beliefs.items():
                distance = util.manhattanDistance(position, pacmanPosition)
                beliefs[position] = value * emissionModel[distance]
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        belief = util.Counter()
        belief.incrementAll(self.particles, 1)
        belief.normalize()
        return belief
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        legalGhostPos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(legalGhostPos)
        self.particles = []
        for i in  range(self.numParticles):
            self.particles.append(legalGhostPos[i % len(legalGhostPos)])
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        if (newParticles.totalCount() == 0):
            ...
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(util.sample(newParticles))
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                distribution = getPositionDistributionForGhost(
